...
当 react 更新发生的时候，Reconciler 会做如下工作：
...
同步更新，没有什么问题 但是为了更快速的响应，假如我们中断操作，交给浏览器绘制，就会出现部分还没有执行 diff 算法，导致页面渲染出现问题（同步更新无法执行中断操作）
...
> 为什么不使用 requestIdleCallback？ 一是兼容性问题，部分浏览器不支持， 二是触发频率不稳定，受很多因素影响（比如浏览器的 tab 切换）
...
generator 足可中断操作，但是不满足第二点，并且 generator 和 async、await 一样存在传染性（即一个 async 函数，调用这个函数的也是 async 函数了），增加运行开销，最重要一点是 Generator 执行的中间状态是上下文关联的
[Generator 执行的中间状态是上下文关联的, 详情解释见 issue](https://github.com/facebook/react/issues/7942#issuecomment-254987818)

> - Generator 执行的中间状态是上下文关联的，上网仔细搜索了一下，给我的感觉是 generator 只能从上次 yield 的地方恢复， 但是此前已经计算的有可能和上面计算的有关，从中断的地方继续开始，之前计算的值就不能复用
> - 但是 fiber 为什么可以呢，fiber（纯函数+memo），如果想要之前的计算失效，只需要将它的 memo 破坏即可，下次重新执行会重新计算被破坏掉的值
...
- 然后 render 时，复用 current Fiber（初始化，所以为 null）创建 workInProgress Fiber，
- commit 阶段将 fiberRootNode 的 current 指针指向 workInProgress Fiber 树使其变为 current Fiber 树
- update 时，新一轮的 render，创建一棵新的 workInProgress Fiber 树（符合条件的可以复用 current Fiber 树对应的节点数据）
- commit 阶段渲染到页面上。渲染完毕后，fiberRootNode 的 current 指针指向 workInProgress Fiber 树， workInProgress Fiber 树变为 current Fiber 树
